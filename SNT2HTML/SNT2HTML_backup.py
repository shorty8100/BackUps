...
class HTMLCreator:
	...
	def DROPDOWNGenerator(self, dicID):
		tempDROP = '<li class="dropdown">'
		for detalhes in dicID:
			if ".snt" in detalhes["destino"]:
				caminho , ficheiro = os.path.split(detalhes["destino"])
				nome , extensao = ficheiro.split(".")
				tempDROP += "<il><a href=" + '"' + nome + ".html" + '">'
				if len(menu["iconOFF"]) > 1:
			...
			if "ID" in detalhes["destino"]:
				...
				if len(menu["iconOFF"]) > 1:
					tempDROP += "<img src='" + self.FileConverter(detalhes["iconOFF"], "png") + "'>   "
				tempDROP += DROPDOWNGenerator(detalhes["destino"])
		...
		if len(destino) > 1:
			...
			if not "POP" in destino:
				...
				if ".csv" in destino:
					...
					tempLINKOBJ += "<nav id='index" + str(index) + "'>\n"
					tempLINKOBJ += '<ul><li class="dropdown"><a href="#"></a><ul class="sub-menu">'
					for menu in dicIDs["ID0"]:
						if ".snt" in menu["destino"]:
							caminho , ficheiro = os.path.split(menu["destino"])
							nome , extensao = ficheiro.split(".")
							tempLINKOBJ += "<il><a href=" + '"' + nome + ".html" + '">'
							if len(menu["iconOFF"]) > 1:
								tempLINKOBJ += "<img src='" + self.FileConverter(menu["iconOFF"], "png") + "'>   "
							tempLINKOBJ += menu["descricao"]
							tempLINKOBJ += "</a></li>"
						if "ID" in menu["destino"]:
							tempLINKOBJ += '<li class="dropdown"><a href="#">'
							if len(menu["iconOFF"]) > 1:
								tempLINKOBJ += "<img src='" + self.FileConverter(menu["iconOFF"], "png") + "'>   "
							tempLINKOBJ += menu["descricao"]
							tempLINKOBJ += "</a>"
							for submenu in dicIDs[menu["destino"]]:
								pass
							tempLINKOBJ += "</li>"
					tempLINKOBJ += "</li></ul>"
					tempLINKOBJ += "</nav>\n"
	...
	def makeContent(self):
		...
		defaultMENU += " nav li ul { background-color:rgb(225,75,75); position:absolute; left:0; top:40px; width:200px; }"
		defaultMENU += " nav li li { position:relative; margin:0; display:block; }"
		defaultMENU += " nav li li ul { position:absolute; top:0; left:200px; margin:0; }"
		defaultMENU += " nav a { color:#fff; text-decoration:none; display:block; }"
		defaultMENU += " nav a:hover, nav a:focus, nav a:active { color:rgb(50,50,50); }"
		defaultMENU += " nav li li a { border-bottom:solid 1px rgb(200,50,50); margin:0 10px; padding:0; }"
		defaultMENU += " nav li li:last-child a { border-bottom:none; }"
		defaultMENU += " nav li.dropdown > a { background-image:url('arrow-down.png'); background-position:right 20px; background-repeat:no-repeat; }"
		defaultMENU += " nav li li.dropdown > a { background-image:url('arrow-right.png'); background-position:right 16px; background-repeat:no-repeat; }"
		...
		tempHEAD += "<script>$(document).ready(function() { $( '.dropdown' ).hover( function(){ $(this).children('.sub-menu').slideDown(0); }, function(){ $(this).children('.sub-menu').slideUp(0);  } ); });</script>"
		tempHEAD += "<title>" + "S-Monitor" + " @ Itelmatis" + "</title>"
		tempHEAD += "</head>\n"
		tempBODY += "</div>\n"
		tempBODY += "</body>\n"
