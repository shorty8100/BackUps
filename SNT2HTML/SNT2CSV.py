...
class HTMLCreator:
	...
	def FileConverter (self, ImagePath, ImageExtensao):
		...
		if extensao.lower() == "bmp":
			...
			if not os.path.isfile("HTML/" + nome + "." + ImageExtensao):
				VideoFileClip(ImagePath).write_gif("HTML/" + nome + "." + ImageExtensao)
		else:
	...
	def makeSTYLE(self):
		...
		tempSTYLE += clearFORMATs + "\n"
		counter = 0
		for sinoticos in self.DicSinopticos:
			tempSTYLE += "#index" + str(counter)
			#print(self.DicSinopticos[sinoticos].ImagemFundo)
			tempSTYLE += "{ background-image: url('" + self.FileConverter(self.DicSinopticos[sinoticos].ImagemFundo, "png") + "');"
	...
	def makeBODY(self):
		...
		for sinoticos in self.DicSinopticos:
			...
			tempBODY += " style='width:100%; height:100%; position:fixed; left:"
			tempBODY += str(self.DicSinopticos[sinoticos].x) + "; top:"
			tempBODY += str(self.DicSinopticos[sinoticos].y) + "; overflow:hidden'>\n"
			counter += 1
			for obj in self.DicSinopticos[sinoticos].objeto:
	...
	def SAVEasHTML(self):
		...
		#tempHTML += self.makeHEAD()
		#tempHTML += self.makeBODY()
